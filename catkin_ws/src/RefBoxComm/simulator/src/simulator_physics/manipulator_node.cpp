...
bool manipulator(simulator::simulator_manipulator::Request  &req ,simulator::simulator_manipulator::Response &res)
{
    ...
    {
	//To modify the range to pick an object modify distancia_min in src/simulator/src/gui/MobileRobotSimulator.py
        if(object_interaction(GRASP,req.object.c_str()))
        {
            printf("object %s grasped\n",req.object.c_str());
            sprintf(str,"%s %s 1",req.action.c_str(),req.object.c_str());
        }
        else
        {
            printf("object %s not grasped\n",req.object.c_str());
            sprintf(str,"%s %s 0",req.action.c_str(),req.object.c_str());
        }
    ...
    else if(strcmp(req.action.c_str(),"drop")==0)
    {
        if(object_interaction(RELEASE,req.object.c_str()))
        {
            ...
            sprintf(str,"dropped %s %f %f %f 1",req.object.c_str(),params.robot_x ,params.robot_y ,params.robot_theta);
            res.answer = str;
            printf("%s\n",str);
        }
        else
        {
            ...
            //sprintf(str,"%s %s %f %f %f 0",req.action.c_str(),req.object.c_str(),params.robot_x ,params.robot_y ,params.robot_theta);
            sprintf(str,"dropped %s %f %f %f 1",req.object.c_str(),params.robot_x ,params.robot_y ,params.robot_theta);
            printf("%s\n",str);
            res.answer = str;
        }
